#!/usr/bin/env node
/**
 * Demo CLI for gawain-stores-plugin
...
import { convertStoresProduct, validateStoresProduct } from './platform/stores_adapter.js';
...
async function main(): Promise<void> {
  console.info('=== Gawain STORES Plugin Demo ===\n');
  ...
  if (!validateStoresProduct(rawProduct)) {
    console.error('Invalid product format. Expected STORES product structure.');
    process.exit(1);
  }

  const productInput = convertStoresProduct(rawProduct);
